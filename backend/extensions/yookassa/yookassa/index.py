...
def handler(event, context):
    ...
    try:
        ...
        cur.execute(f"""
            ...
            (order_number, user_name, user_email, user_phone, amount, status, application_id, order_type, items_data, delivery_address, order_comment, user_id, created_at, updated_at)
            VALUES (%s, %s, %s, %s, %s, 'pending', %s, %s, %s, %s, %s, %s, %s, %s)
            RETURNING id
        """, (order_number, user_name, user_email, user_phone, amount, application_id, order_type, json.dumps(items_data, ensure_ascii=False) if items_data is not None else None, delivery_address or None, order_comment or None, user_id, now, now))
