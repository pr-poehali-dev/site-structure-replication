...
def compute_live_times(game):
    """Пересчитывает оставшееся время с учётом того, сколько прошло с последнего хода."""
...
def handler(event: dict, context) -> dict:
    ...
    if method == 'POST' and action == 'move':
        ...
        try:
            ...
            conn.close()
            return {'statusCode': 400, 'headers': cors_headers(), 'body': json.dumps({'error': 'Некорректный ход'})}
        ...
        cur.execute(
            ...
            (board.fen(), new_pgn, new_turn, white_ms, black_ms, game_id)
        )
        ...
        if board.is_checkmate():
            ...
            finish_game(cur, game_id, '1/2-1/2', FIFTY_MOVE)
        elif board.can_claim_threefold_repetition():
            finish_game(cur, game_id, '1/2-1/2', REPETITION)
